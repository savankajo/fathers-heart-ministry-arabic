...
type PodcastEpisode = { id: string; title: string };
...
const PodcastCard: React.FC<{ episode: PodcastEpisode }> = ({ episode }) => <a href={getEpisodeUrl(episode.id)} target="_blank" rel="noopener noreferrer" className="group block overflow-hidden rounded-2xl bg-white shadow-lg border border-gray-100 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl"><div className="relative aspect-video overflow-hidden bg-[#1a3a5c]"><img src={getThumbnailUrl(episode.id)} alt={episode.title} loading="lazy" className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" onError={e => { (e.target as HTMLImageElement).src = 'https://res.cloudinary.com/dyjffxbef/image/upload/v1765302297/logotransport_ljm5vs.png'; }} /><div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/10 to-transparent opacity-80" /><div className="absolute left-4 bottom-4 flex h-12 w-12 items-center justify-center rounded-full bg-[#D4AF37] text-black shadow-lg transition-transform duration-300 group-hover:scale-110"><svg className="ml-0.5 h-6 w-6" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path d="M8 5v14l11-7z" /></svg></div></div><div className="p-5"><h2 className="font-heading text-base font-bold leading-snug text-[#1a3a5c] transition-colors duration-200 group-hover:text-[#b99321]">{episode.title}</h2><p className="mt-4 inline-flex items-center gap-2 text-sm font-bold text-red-600">Watch on YouTube</p></div></a>;
const PodcastPage: React.FC = () => { const [searchTerm, setSearchTerm] = useState(''); const filteredEpisodes = useMemo(() => { const query = searchTerm.trim().toLowerCase(); if (!query) return PODCAST_EPISODES; return PODCAST_EPISODES.filter(episode => episode.title.toLowerCase().includes(query)); }, [searchTerm]); return <div className="font-body bg-gray-50 min-h-screen"><SEO title="Podcast — Coffee With the Shepherd" description="Watch Coffee With the Shepherd podcast episodes from Father's Heart Ministry on YouTube." url="https://fathersheartministry.ca/#/podcast" /><div className="bg-[#1a3a5c] py-16 text-center"><p className="text-[#D4AF37] font-semibold tracking-[0.3em] text-sm uppercase mb-3">Podcast Episodes</p><h1 className="text-4xl sm:text-5xl font-bold font-heading text-white">Coffee With the Shepherd</h1><div className="w-16 h-1 bg-[#D4AF37] mx-auto mt-4 rounded-full" /><p className="text-white/75 mt-4 max-w-2xl mx-auto px-4">Watch every public episode from the Coffee With the Shepherd YouTube channel.</p></div><section className="py-14"><div className="container mx-auto px-4 max-w-6xl"><div className="mb-10 overflow-hidden rounded-2xl bg-white shadow-xl border border-gray-100"><div className="bg-gradient-to-r from-[#1a3a5c] to-[#0f2540] p-7 text-white flex flex-col md:flex-row items-center gap-6"><div className="w-24 h-24 rounded-full overflow-hidden flex-shrink-0 border-4 border-[#D4AF37] bg-white shadow-xl"><img src="https://res.cloudinary.com/dyjffxbef/image/upload/v1765302297/logotransport_ljm5vs.png" alt="Father's Heart Ministry" className="w-full h-full object-contain p-3" /></div><div className="text-center md:text-left"><h2 className="text-2xl font-bold font-heading mb-2">Coffee With the Shepherd on YouTube</h2><p className="text-white/75 text-sm leading-relaxed max-w-2xl">Select an episode below to open it directly on YouTube. The episode images use the YouTube thumbnails from your channel.</p></div></div><div className="p-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between"><div className="relative w-full md:max-w-md"><input type="text" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="Search episodes" className="w-full rounded-full border-2 border-gray-200 bg-white px-5 py-3 pr-11 text-gray-700 shadow-sm transition-all placeholder-gray-400 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-[#1a3a5c]" /></div><a href={YOUTUBE_CHANNEL_URL} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 rounded-full bg-red-600 px-6 py-3 text-sm font-bold text-white shadow-md transition-all duration-300 hover:-translate-y-0.5 hover:bg-red-700">Visit Channel</a></div></div><p className="mb-8 text-center text-sm text-gray-500">Showing {filteredEpisodes.length} of {PODCAST_EPISODES.length} episodes</p>{filteredEpisodes.length > 0 ? <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">{filteredEpisodes.map(episode => <PodcastCard key={episode.id} episode={episode} />)}</div> : <div className="mx-auto max-w-2xl rounded-xl bg-white py-16 text-center shadow-inner"><p className="text-xl font-medium text-gray-500">No episodes found.</p></div>}</div></section></div>; };
